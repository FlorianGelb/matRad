classdef MatRad_Config < handle
    ...
    methods (Access = private)
        function obj = MatRad_Config()
            ...
            %Set default Log Types
            logTypeRegistry = MatRad_LogTypeRegistry.instance();
            logTypeRegistry.RegisteredLogTypes = matRad_LogType(3, "info", "", @(m)fprintf(m));
            logTypeRegistry.RegisteredLogTypes = matRad_LogType(5, "debug", "DEBUG: ", @(m)fprintf(m));
            logTypeRegistry.RegisteredLogTypes = matRad_LogType(4, "dep", "DEPRECATION WARNING:", @(m)warning(m));
            logTypeRegistry.RegisteredLogTypes = matRad_LogType(2, "warning", "", @(m)warning(m));
            logTypeRegistry.RegisteredLogTypes = matRad_LogType(1, "error", "matRad:Error", @(m)error(m));
    ...
    methods
        ...
        function logWithCustomLogType(obj, logTypeName, message)
            logTypeRegistry = MatRad_LogTypeRegistry.instance();
            try
                logType = logTypeRegistry.getRegisteredLogTypeByName(logTypeName);
                if obj.logLevel >= logType.logLevel
                    logType.action(logType.modifiedMessage + message);
                end
            catch ME
